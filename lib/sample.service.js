 ...
 * @param {object} type of the recommendation to include, null means wildcard
 */
const filterRecommendations = (dnr, itemToWeightMap, type, recommendation) => {
    ...
    //category filter
    if (type != null && type.indexOf(recommendation.itemMetadata.type) < 0) return false;
    // TODO Determine what these numbers / thresholds should be. Also, make them configurable
    return recommendation.weight > 0.1;
};
 ...
 */
const sampleRecommendationsForUser = (userId, numberOfSamples = 20, type = null) => {
    ...
    return Promise.all([recommendationPromise, itemWeightsPromise, doNotRecommendPromise])
        .then(([userRecommendationsObj, userItemWeights, dnr]) => {
            ...
            var filteredRecommendations = _.filter(allRecommendations, filterRecommendations.bind(this, dnr, itemIdToWeightMap, type));
